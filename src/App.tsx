...
export default function App(){
  useEffect(() => {
    telegramAnalytics.init({
      token: 'eyJhcHBfbmFtZSI6ImZvcnR5N211c2ljIiwiYXBwX3VybCI6Imh0dHBzOi8vdC5tZS9mb3J0eTdtYm90IiwiYXBwX2RvbWFpbiI6Imh0dHBzOi8vbXktdGVsZWdyYW0tbXVzaWMtbWluaS1hcHAucGFnZXMuZGV2LyJ9!VFb2osRpPyTsckRd52kuxtTCKe2UOkzMkuTUdxTs1U4=',
      appName: 'forty7music',
    });
  }, []);
...
const openAlbum=async(id:string,title:string,artist:string,cover:string)=>{
  ...
  const matchesArtist=(x:ArtistInfo,a:ArtistInfo)=>{
    if((x.source||'')!==(a.source||''))return false;
    if(x.id&&a.id)return x.id===a.id;
    return x.name===a.name;
  };
  const isFavA=(a:ArtistInfo)=>favArtists.some(x=>matchesArtist(x,a));
  const toggleFavA=(a:ArtistInfo)=>{setFavArtists(prev=>{const has=prev.some(x=>matchesArtist(x,a));const n=has?prev.filter(x=>!matchesArtist(x,a)):[{...a,latestRelease:null,tracks:[],albums:[]},...prev];try{localStorage.setItem('fa47',JSON.stringify(n));}catch{}triggerSync(liked,playlists,history,volume,n,favAlbums,blockedArtists,bgCover);return n;});};
    ...
    <div className={`fp-root ${['tdesktop','macos','weba','webk'].includes(window.Telegram?.WebApp?.platform??'')?'fp-desktop':''}`} style={{position:'relative' as const,background:BG,height:'100vh',width:'100%',display:'flex',flexDirection:'column',justifyContent:'space-between',alignItems:'center',padding:'0 22px',fontFamily:"-apple-system,'SF Pro Display',sans-serif",boxSizing:'border-box',overflow:'hidden',animation:'fadeIn 0.3s ease'}}>
      ...
      <style>{`
        ...
        *{-webkit-tap-highlight-color:transparent}
        ...
        .fp-desktop .fp-controls>button:first-child,.fp-desktop .fp-controls>button:last-child{transform:scale(0.92)!important}
  ...
  <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',flexDirection:'column' as const,justifyContent:'flex-end',background:'rgba(0,0,0,0.4)',animation:'fadeIn 0.2s ease'}} onPointerDown={()=>setShowFxPanel(false)}>
    <div style={{background:'rgba(14,14,14,0.55)',backdropFilter:'blur(40px)',WebkitBackdropFilter:'blur(40px)',borderRadius:'24px 24px 0 0',padding:'20px 20px 40px',width:'100%',maxHeight:'85vh',overflowY:'auto' as const,border:'1px solid rgba(255,255,255,0.07)',borderBottom:'none'}} onPointerDown={e=>e.stopPropagation()}>
      <div style={{display:'flex',justifyContent:'flex-end',marginBottom:12}}>
        <button onPointerDown={e=>{e.stopPropagation();setShowFxPanel(false);}} style={{background:'none',border:'none',cursor:'pointer',color:TEXT_SEC,fontSize:28,padding:'2px 8px',lineHeight:1,...tap}}>×</button>
      </div>
      {!subActive&&(
...
{showEqPanel&&(
      ...
      <div className="fp-controls" style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,marginTop:'2vh',marginBottom:'2.5vh',animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.18s both'}}>
        <button onPointerDown={()=>{if(!showFxPanel)fetch('https://eqq.onrender.com/health').catch(()=>{});setShowFxPanel(v=>!v);}} style={{background:'none',border:'none',cursor:'pointer',padding:4,opacity:showFxPanel?0.90:0.8,transition:'opacity 0.2s ease',...tap}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={showFxPanel?ACC:'rgba(255,255,255,0.95)'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke 0.2s ease'}}><line x1="4" y1="21" x2="4" y2="14"/><line x1="4" y1="10" x2="4" y2="3"/><line x1="12" y1="21" x2="12" y2="12"/><line x1="12" y1="8" x2="12" y2="3"/><line x1="20" y1="21" x2="20" y2="16"/><line x1="20" y1="12" x2="20" y2="3"/><line x1="1" y1="14" x2="7" y2="14"/><line x1="9" y1="8" x2="15" y2="8"/><line x1="17" y1="16" x2="23" y2="16"/></svg>
        </button>
        <button className="prev-next-btn" onPointerDown={e=>{e.preventDefault();if(fxLoading)return;playPrev();}} style={{background:'none',border:'none',cursor:'pointer',padding:6,opacity:fxLoading?0.2:playHistory.length>0?1:0.4,...tap}}>
          <svg width="30" height="30" viewBox="0 0 24 24" fill="#fff" stroke="#fff" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><path d="M19 20L9 12L19 4V20Z"/><rect x="4" y="5" width="2" height="14" rx="1"/></svg>
        </button>
        <button className="play-btn" onPointerDown={togglePlay} style={{width:'clamp(64px,17vw,78px)',height:'clamp(64px,17vw,78px)',borderRadius:'50%',background:ACC,border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0,boxShadow:`0 5px 20px ${ACC}66`,transition:'transform 0.15s ease',...tap}}>
          <PP sz="lg"/>
        </button>
        <button className="prev-next-btn" onPointerDown={e=>{e.preventDefault();if(fxLoading)return;playNext();}} style={{background:'none',border:'none',cursor:'pointer',padding:6,opacity:fxLoading?0.2:(queue.length>0||recs.length>0||history.length>0)?1:0.4,...tap}}>
        ...
        <button onPointerDown={()=>setLoop(!loop)} style={{background:'none',border:'none',cursor:'pointer',padding:6,opacity:loop?0.90:0.8,transition:'opacity 0.2s ease',...tap}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={loop?ACC:'rgba(255,255,255,0.95)'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke 0.2s ease'}}><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 014-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 01-4 4H3"/></svg>
        </button>
      </div>
                    ...
                    <div key={tr.id} className="press-scale" style={{flexShrink:0,width:148,cursor:'pointer',padding:10,borderRadius:14,background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,255,255,0.05)'}} onClick={()=>playFromRecent(tr,history.slice(0,12))}>
                      <div style={{position:'relative' as const,width:128,height:128,borderRadius:10,overflow:'hidden',marginBottom:8,background:BG3}}>
                        ...
                        <button onPointerDown={e=>{e.stopPropagation();}} onClick={e=>{e.stopPropagation();e.preventDefault();addQ(tr,e as any);}} style={{position:'absolute' as const,top:7,right:7,width:28,height:28,borderRadius:8,background:'rgba(20,20,20,0.55)',backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)',border:'1px solid rgba(255,255,255,0.1)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',padding:0,...tap}}>
                          <svg viewBox="0 0 147 112" style={{width:14,height:14,transition:'fill 0.2s ease'}} fill={inQ(tr.id)?ACC:'#fff'}><path d="M80.5 0C86.299 0 91 4.70101 91 10.5V18H29.5C23.701 18 19 22.701 19 28.5V56H10.5C4.70101 56 0 51.299 0 45.5V10.5C0 4.70101 4.70101 0 10.5 0H80.5Z"/><path d="M108.5 28C114.299 28 119 32.701 119 38.5V46H58.5C52.701 46 48 50.701 48 56.5V84H38.5C32.701 84 28 79.299 28 73.5V38.5C28 32.701 32.701 28 38.5 28H108.5Z"/><path d="M136.5 56H66.5C60.701 56 56 60.701 56 66.5V101.5C56 107.299 60.701 112 66.5 112H136.5C142.299 112 147 107.299 147 101.5V66.5C147 60.701 142.299 56 136.5 56Z"/></svg>
                        </button>
                      </div>
...
<button key={tab.id} onPointerDown={()=>setLibTab(tab.id as any)}
 ...
 {libTab==='liked'&&(liked.length===0
    ...
    {libTab==='artists'&&(<div style={{padding:'0 16px'}}>{favArtists.length===0?<div style={{display:'flex',flexDirection:'column',alignItems:'center',paddingTop:60,animation:'fadeIn 0.3s ease'}}><div style={{fontSize:38,marginBottom:12}}>🎤</div><div style={{fontSize:13,color:TEXT_MUTED}}>{lang==='ru'?'Нет избранных артистов':'No favourite artists'}</div></div>:favArtists.map(a=>(<div key={a.id||a.name} onClick={()=>openArtist(a.permalink||'',a.name,a.avatar||'',a.followers,a.source)} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:`1px solid #1e1e1e`,cursor:'pointer',transition:'opacity 0.15s ease',...tap}}><Img src={a.avatar||''} size={46} radius={23}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,color:TEXT_PRIMARY}}>{a.name}</div>{a.username&&<div style={{fontSize:10,color:TEXT_SEC,marginTop:1}}>@{a.username}</div>}{a.followers>0&&<div style={{fontSize:10,color:TEXT_SEC,marginTop:1}}>{fmtP(a.followers)} {lang==='ru'?'подписчиков':'followers'}</div>}</div><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#4a4a4a" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg></div>))}</div>)}
          ...
          <div style={{
            ...
            <div style={{display:'flex',alignItems:'center',gap:0,flexShrink:0,marginTop:2}}>
              <button className="prev-next-btn"
                ...
                <svg width={['tdesktop','macos','weba','webk'].includes(window.Telegram?.WebApp?.platform??'')?21:18} height={['tdesktop','macos','weba','webk'].includes(window.Telegram?.WebApp?.platform??'')?21:18} viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="19 20 9 12 19 4 19 20"/><line x1="5" y1="19" x2="5" y2="5"/></svg>
              </button>
              <button className="prev-next-btn"
                ...
                <svg width={['tdesktop','macos','weba','webk'].includes(window.Telegram?.WebApp?.platform??'')?21:18} height={['tdesktop','macos','weba','webk'].includes(window.Telegram?.WebApp?.platform??'')?21:18} viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 4 15 12 5 20 5 4"/><line x1="19" y1="5" x2="19" y2="19"/></svg>
              </button>
            </div>
