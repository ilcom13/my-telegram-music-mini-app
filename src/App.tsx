...
{showEqPanel&&(
      ...
      <div className="fp-controls" style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,marginTop:'2vh',marginBottom:'2.5vh',animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.18s both'}}>
        <button onPointerDown={()=>{if(!showFxPanel)fetch('https://eqq.onrender.com/health').catch(()=>{});setShowFxPanel(v=>!v);}} style={{background:'none',border:'none',cursor:'pointer',padding:4,opacity:0.25,transition:'opacity 0.2s ease',...tap}}>
        ...
        <button onPointerDown={()=>setLoop(!loop)} style={{background:'none',border:'none',cursor:'pointer',padding:6,opacity:0.25,transition:'opacity 0.2s ease',...tap}}>
