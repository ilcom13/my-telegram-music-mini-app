...
export default function App(){
  ...
  const[blockedArtists,setBlockedArtists]=useState<string[]>([]); // #6
  ...
  const[playHistory,setPlayHistory]=useState<Track[]>([]); // #5 история воспроизведения для кнопки назад
  ...
  // Умные рекомендации — только артисты с >= 2 прослушиваниями, фильтруем заблокированных
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    const onE=()=>{
      if(loop){a.currentTime=0;a.play();}
        ...
        const available=pool.filter(tr=>tr.mp3&&tr.id!==current?.id&&!blockedArtists.includes(tr.artist));
  ...
  const playDirect=(track:Track)=>{
    ...
    // #5 сохраняем текущий в историю воспроизведения
  ...
  const playPrev=()=>{
    if(playHistory.length>0){
      ...
      // Нет истории — просто перемотать в начало
      audio.current.currentTime=0;
    }
  };

  // #5 — перейти к следующему треку
  const playNext=()=>{
    if(queue.length>0){
      const nxt=queue[0];
  ...
  // #6 блокировка артиста в рекомендациях
  ...
  const openArtist=async(permalink:string,name:string,avatar:string,followers:number)=>{
    ...
    try{
      ...
      if(userId){
        ...
        // Сразу показываем треки из /artist (они уже есть), потом подгружаем остальные
  ...
  const loadArtistTracks=async(userId:string,offset:number,reset=false)=>{
    ...
    try{
      ...
      setArtistTracks(prev=>{
        if(reset){
          // Объединяем с теми что уже показаны (из /artist), убираем дубли
  ...
  const HBtn=({track,sz=19}:{track:Track;sz?:number})=>(
    <button onClick={e=>toggleLike(track,e)} style={{background:'none',border:'none',cursor:'pointer',padding:4,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',outline:'none'}}>
        ...
        <div onClick={e=>{e.stopPropagation();if(mOpen){setMenuId(null);return;}setMenuId(null);playTrack(track);}} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',borderRadius:12,cursor:'pointer',marginBottom:1,background:active?ACC_DIM:'transparent'}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:4,marginTop:2}}>
              {!track.isArtist&&!track.isAlbum&&onArtistClick
                ?<button onClick={e=>{e.stopPropagation();onArtistClick(track.artist,track.cover);}} style={{background:'none',border:'none',padding:0,cursor:'pointer',fontSize:11,color:TEXT_SEC,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:130,textAlign:'left',outline:'none'}}>{track.artist}</button>
          ...
          {!track.isArtist&&!track.isAlbum&&(
            <div style={{display:'flex',alignItems:'center',gap:1,flexShrink:0}}>
              <button onClick={e=>addQ(track,e)} style={{background:'none',border:'none',cursor:'pointer',padding:'3px',display:'flex',alignItems:'center',justifyContent:'center',outline:'none'}}>
              ...
              <button onClick={e=>{e.stopPropagation();setMenuId(mOpen?null:track.id);}} style={{background:'none',border:'none',cursor:'pointer',padding:'3px',display:'flex',alignItems:'center',justifyContent:'center',outline:'none'}}>
        ...
        {mOpen&&(
          <div onClick={e=>e.stopPropagation()} style={{position:'absolute',right:8,top:'calc(100%+2px)',background:'#222',border:'1px solid #2a2a2a',borderRadius:12,zIndex:50,minWidth:174,boxShadow:'0 12px 32px rgba(0,0,0,0.8)',overflow:'hidden'}}>
            {menuItems.map((item,i)=>(
              <button key={i} onClick={item.fn} style={{display:'flex',alignItems:'center',gap:9,width:'100%',padding:'10px 12px',background:'none',border:'none',cursor:'pointer',color:i===menuItems.length-1&&showBlockBtn?'#d06060':'#ddd',fontSize:12,borderBottom:i<menuItems.length-1?'1px solid #2a2a2a':'none',textAlign:'left' as const,outline:'none'}}>
  ...
  const BackBtn=()=>(
    <button onClick={()=>setScreen(prevScreen.current)} style={{background:'none',border:'none',cursor:'pointer',padding:'4px 8px 4px 0',display:'flex',alignItems:'center',gap:5,outline:'none',WebkitTapHighlightColor:'transparent' as unknown as string}}>
  ...
  // Кнопки Prev/Next для плеера
  const PrevBtn=({size=22}:{size?:number})=>(
    <button onClick={playPrev} style={{background:'none',border:'none',cursor:'pointer',padding:4,opacity:playHistory.length>0?1:0.35,outline:'none'}}>
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round"><polygon points="19 20 9 12 19 4 19 20"/><line x1="5" y1="19" x2="5" y2="5"/></svg>
    </button>
  );
  const NextBtn=({size=22}:{size?:number})=>(
    <button onClick={playNext} style={{background:'none',border:'none',cursor:'pointer',padding:4,opacity:(queue.length>0||recs.length>0||history.length>0)?1:0.35,outline:'none'}}>
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round"><polygon points="5 4 15 12 5 20 5 4"/><line x1="19" y1="5" x2="19" y2="19"/></svg>
    </button>
  );
  ...
  if(fullPlayer&&current)return(
    <div style={{background:FULL_BG,height:'100vh',width:'100%',display:'flex',flexDirection:'column',alignItems:'center',padding:'0 22px',fontFamily:"-apple-system,'SF Pro Display',sans-serif",boxSizing:'border-box',overflow:'hidden'}}>
      ...
      <div style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',paddingTop:40,paddingBottom:10,flexShrink:0}}>
        <button onClick={()=>setFullPlayer(false)} style={{background:'none',border:'none',cursor:'pointer',padding:6,margin:-6,outline:'none'}}>
        ...
        <button onClick={()=>setShowQueue(true)} style={{background:'none',border:'none',cursor:'pointer',padding:6,margin:-6,position:'relative',outline:'none'}}>
      ...
      <div style={{width:'100%',flexShrink:0,marginBottom:10}}>
        {/* #1 название — просто текст, не кликабельно */}
      ...
      {/* #5 Кнопки Prev / Play / Next — все рабочие */}
  ...
  return(
    <div onClick={()=>menuId&&setMenuId(null)} style={{background:BG,minHeight:'100vh',width:'100%',fontFamily:"-apple-system,'SF Pro Display',sans-serif",position:'relative',boxSizing:'border-box'}}>
      <audio ref={audio}/>
      <style>{`button:focus,button:active{outline:none!important;box-shadow:none!important}*{-webkit-tap-highlight-color:transparent}::-webkit-scrollbar{display:none}`}</style>
      <div style={{paddingBottom:current?NAV_H+80+8:NAV_H+6,minHeight:'100vh'}}>

        {/* ── ARTIST ───────────────────────────────────────────────────────────── */}
        {screen==='artist'&&(
          <div>
            {/* #1 убраны лишние отступы — кнопка назад прямо в topPadding */}
            <div style={{paddingTop:44,paddingLeft:16,paddingRight:16,paddingBottom:0}}>
              <BackBtn/>
            </div>
            {artistLoading?<Spinner/>:artistPage&&(
              <div>
                {/* Баннер без отступов сверху */}
                <div style={{width:'100%',height:110,background:BG3,position:'relative',overflow:'hidden',marginTop:6,marginBottom:-32}}>
                ...
                <div style={{padding:'0 16px 12px',display:'flex',alignItems:'flex-end',gap:12,position:'relative',zIndex:1}}>
                  <div style={{flexShrink:0,borderRadius:'50%',overflow:'hidden',border:`2px solid ${BG}`}}><Img src={artistPage.avatar} size={68} radius={34}/></div>
                ...
                {/* #3 Последний релиз ВЫШЕ дискографии */}
                ...
                <div style={{padding:'0 16px 16px'}}>
                  ...
                  <div style={{display:'flex',gap:6,marginBottom:12}}>
                    {(['albums','tracks'] as const).map(tab=>(
                      <button key={tab} onClick={()=>{setArtistTab(tab);if(tab==='tracks'&&artistTracks.length===0&&artistUserId.current)loadArtistTracks(artistUserId.current,0,true);}} style={{padding:'5px 16px',borderRadius:16,border:'none',background:artistTab===tab?ACC:ACC_DIM,color:artistTab===tab?BG:ACC,fontSize:12,fontWeight:artistTab===tab?600:400,cursor:'pointer',outline:'none'}}>
                  ...
                  {artistTab==='tracks'&&(
                    ...
                    <div>
                      {artistTracks.length===0?<div style={{textAlign:'center',color:TEXT_MUTED,fontSize:12,padding:'16px 0'}}>{t('noTracks')}</div>:
                        <div style={{padding:'0 4px'}}>
                      ...
                      {artistTracksHasMore&&!artistTracksLoading&&(
                        <div style={{display:'flex',justifyContent:'center',padding:'10px 0'}}>
                          <button onClick={()=>loadArtistTracks(artistUserId.current,artistTracksOffset)} style={{padding:'9px 28px',background:ACC_DIM,border:`1px solid ${ACC}22`,borderRadius:11,color:ACC,fontSize:12,cursor:'pointer',outline:'none'}}>{t('showMore')}</button>
                        </div>
                      )}
                    </div>
                  )}
              ...
              <div>
                <div style={{padding:'10px 16px 0',display:'flex',gap:14,alignItems:'center'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <button onClick={()=>openArtist('',albumPage.artist,'',0)} style={{background:'none',border:'none',padding:0,cursor:'pointer',marginTop:4,display:'block',textAlign:'left' as const,outline:'none'}}><span style={{fontSize:12,color:ACC}}>{albumPage.artist}</span></button>
                ...
                <div style={{display:'flex',gap:7,padding:'12px 16px'}}>
                  ...
                  <button onClick={()=>toggleFavAl(albumPage)} style={{padding:'10px 14px',borderRadius:10,border:`1px solid ${isFavAl(albumPage.id)?ACC:'#3a3a3a'}`,background:isFavAl(albumPage.id)?ACC_DIM:'transparent',color:isFavAl(albumPage.id)?ACC:TEXT_SEC,fontSize:15,cursor:'pointer',outline:'none'}}>{isFavAl(albumPage.id)?'♥':'♡'}</button>
                </div>
        ...
        {screen==='home'&&(
          <div>
                  ...
                  {history.slice(0,8).map(tr=>(
                    <div key={tr.id} style={{width:100,borderRadius:10,background:BG2,overflow:'hidden',cursor:'pointer',flexShrink:0,border:'1px solid #1e1e1e'}}>
                      ...
                      <div style={{padding:'6px 7px 8px',boxSizing:'border-box' as const}}>
                        <button onClick={()=>openArtist('',tr.artist,tr.cover,0)} style={{background:'none',border:'none',padding:0,cursor:'pointer',display:'block',width:'100%',textAlign:'left' as const,outline:'none'}}>
            ...
            {recs.length===0&&history.length<2
              ...
              // #6 showBlockBtn=true для рекомендаций
        ...
        {screen==='library'&&(
          <div>
            ...
            {libTab==='liked'&&(liked.length===0?<div style={{display:'flex',flexDirection:'column',alignItems:'center',paddingTop:60}}><div style={{fontSize:38,marginBottom:12,filter:'sepia(1) saturate(3) hue-rotate(5deg)'}}>🎵</div><div style={{fontSize:13,color:TEXT_MUTED}}>{t('noLiked')}</div></div>:<div style={{padding:'0 4px'}}>{liked.map((tr,i)=><TRow key={tr.id} track={tr} num={i+1} onArtistClick={(n,c)=>openArtist('',n,c,0)}/>)}</div>)}
            {libTab==='artists'&&(<div style={{padding:'0 16px'}}>{favArtists.length===0?<div style={{display:'flex',flexDirection:'column',alignItems:'center',paddingTop:60}}><div style={{fontSize:38,marginBottom:12,filter:'sepia(1) saturate(3) hue-rotate(5deg)'}}>🎤</div><div style={{fontSize:13,color:TEXT_MUTED}}>{lang==='ru'?'Нет избранных артистов':'No favourite artists'}</div></div>:favArtists.map(a=>(<div key={a.id||a.name} onClick={()=>openArtist(a.permalink||'',a.name,a.avatar||'',a.followers)} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:`1px solid #1e1e1e`,cursor:'pointer'}}><Img src={a.avatar||''} size={46} radius={23}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,color:TEXT_PRIMARY}}>{a.name}</div>{a.username&&<div style={{fontSize:10,color:TEXT_SEC,marginTop:1}}>@{a.username}</div>}{a.followers>0&&<div style={{fontSize:10,color:TEXT_SEC,marginTop:1}}>{fmtP(a.followers)} {lang==='ru'?'подписчиков':'followers'}</div>}</div><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#4a4a4a" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg></div>))}</div>)}
            {libTab==='albums'&&(<div style={{padding:'0 16px'}}>{favAlbums.length===0?<div style={{display:'flex',flexDirection:'column',alignItems:'center',paddingTop:60}}><div style={{fontSize:38,marginBottom:12,filter:'sepia(1) saturate(3) hue-rotate(5deg)'}}>💿</div><div style={{fontSize:13,color:TEXT_MUTED}}>{lang==='ru'?'Нет избранных альбомов':'No favourite albums'}</div></div>:favAlbums.map(al=>(<div key={al.id} onClick={()=>openAlbum(al.id,al.title,al.artist,al.cover)} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:`1px solid #1e1e1e`,cursor:'pointer'}}><Img src={al.cover} size={50} radius={8}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,color:TEXT_PRIMARY,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{al.title}</div><div style={{fontSize:11,color:TEXT_SEC,marginTop:2}}>{al.artist}</div></div><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#4a4a4a" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg></div>))}</div>)}
            {libTab==='playlists'&&(
              <div style={{padding:'0 16px'}}>
                ...
                {showNewPl&&(<div style={{background:BG2,border:'1px solid #242424',borderRadius:11,padding:'11px',marginBottom:9}}><input autoFocus placeholder={t('playlistName')} value={newPlName} onChange={e=>setNewPlName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&createPl()} style={{width:'100%',padding:'8px 11px',fontSize:13,background:BG,border:'1px solid #2a2a2a',borderRadius:7,color:TEXT_PRIMARY,outline:'none',boxSizing:'border-box' as const,marginBottom:7}}/><div style={{display:'flex',gap:6}}><button onClick={createPl} style={{flex:1,padding:'8px',background:ACC,border:'none',borderRadius:7,color:BG,fontSize:12,fontWeight:600,cursor:'pointer',outline:'none'}}>{t('create')}</button><button onClick={()=>{setShowNewPl(false);setNewPlName('');}} style={{flex:1,padding:'8px',background:BG3,border:'none',borderRadius:7,color:TEXT_SEC,fontSize:12,cursor:'pointer',outline:'none'}}>{t('cancel')}</button></div></div>)}
                {playlists.map(pl=>{const isOpen=openPlId===pl.id;return(
                  <div key={pl.id} style={{background:BG2,border:'1px solid #1e1e1e',borderRadius:12,marginBottom:7,overflow:'hidden'}}>
                    <div onClick={()=>setOpenPlId(isOpen?null:pl.id)} style={{padding:'11px 13px',cursor:'pointer',display:'flex',alignItems:'center',gap:10}}>
                      ...
                      <button onClick={e=>{e.stopPropagation();playPl(pl);}} style={{width:34,height:34,minWidth:34,borderRadius:'50%',background:ACC,border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0,padding:0,outline:'none'}}><div style={{width:0,height:0,borderStyle:'solid',borderWidth:'6px 0 6px 10px',borderColor:`transparent transparent transparent ${BG}`,marginLeft:3}}/></button>
                    ...
                    <div style={{borderTop:'1px solid #1e1e1e'}}>{pl.tracks.length===0?<div style={{padding:'14px',textAlign:'center',color:TEXT_MUTED,fontSize:11}}>{lang==='ru'?'Нет треков':'No tracks'}</div>:pl.tracks.map((tr,i)=>(<div key={tr.id+i} onClick={()=>{playTrack(tr);setQueue(pl.tracks.slice(i+1));}} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 13px',cursor:'pointer',background:current?.id===tr.id?ACC_DIM:'transparent',borderBottom:'1px solid #1a1a1a'}}><Img src={tr.cover} size={36} radius={6}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,color:current?.id===tr.id?ACC:TEXT_PRIMARY,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{tr.title}</div><div style={{fontSize:10,color:TEXT_SEC,marginTop:1}}>{tr.artist}</div></div><div style={{fontSize:10,color:TEXT_SEC}}>{tr.duration}</div></div>))}</div></div>)}
                  </div>
                );})}
              </div>
            )}
        ...
        {screen==='trending'&&(()=>{const ct=trends[trendGenre]||[];return(
          <div>
            ...
            <div style={{display:'flex',gap:6,padding:'0 16px 12px',overflowX:'auto'}}>
              {GENRES.map(g=>(<button key={g.id} onClick={()=>setTrendGenre(g.id)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:3,padding:'8px 11px',borderRadius:12,border:trendGenre===g.id?`1px solid ${ACC}44`:'1px solid #222',background:trendGenre===g.id?ACC_DIM:BG2,color:trendGenre===g.id?ACC:TEXT_SEC,cursor:'pointer',flexShrink:0,minWidth:54,transition:'all 0.15s',outline:'none'}}><span style={{fontSize:19}}>{g.e}</span><span style={{fontSize:9,fontWeight:trendGenre===g.id?600:400,whiteSpace:'nowrap'}}>{g.label}</span></button>))}
            </div>
            {trendLoading&&ct.length===0?<Spinner/>:ct.length===0?<div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'60px 20px 0',textAlign:'center'}}><div style={{fontSize:38,marginBottom:12}}>📈</div><div style={{fontSize:13,color:TEXT_MUTED}}>{t('notFound')}</div><button onClick={()=>loadTrend(trendGenre,true)} style={{marginTop:12,padding:'8px 20px',background:ACC_DIM,border:'none',borderRadius:9,color:ACC,fontSize:12,cursor:'pointer',outline:'none'}}>{t('retry')}</button></div>:<div><div style={{padding:'0 4px'}}>{ct.map((tr,i)=><TRow key={tr.id+i} track={tr} num={i+1} onArtistClick={(n,c)=>openArtist('',n,c,0)}/>)}</div><div style={{padding:'10px 16px 6px',display:'flex',justifyContent:'center'}}><button onClick={()=>loadTrend(trendGenre,false)} disabled={trendLoading} style={{padding:'9px 32px',background:ACC_DIM,border:`1px solid ${ACC}22`,borderRadius:11,color:ACC,fontSize:12,cursor:trendLoading?'not-allowed':'pointer',opacity:trendLoading?.5:1,outline:'none'}}>{trendLoading?t('loading'):t('loadMore')}</button></div></div>}
          </div>
        );})()}
        ...
        {screen==='profile'&&(
          <div>
            <div style={{paddingTop:44,paddingLeft:16,paddingRight:16,paddingBottom:18,display:'flex',alignItems:'center',gap:4}}>
              <button onClick={()=>setScreen('home')} style={{background:'none',border:'none',cursor:'pointer',padding:'4px 8px 4px 0',display:'flex',alignItems:'center',outline:'none'}}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={TEXT_SEC} strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg></button>
            ...
            <div style={{padding:'0 16px'}}>
              ...
              {blockedArtists.length>0&&<button onClick={()=>{setBlockedArtists([]);try{localStorage.setItem('ba47',JSON.stringify([]));}catch{}}} style={{width:'100%',marginTop:4,padding:'8px',background:'#1a0a0a',border:'1px solid #2a1010',borderRadius:9,color:'#c05050',fontSize:11,cursor:'pointer',outline:'none'}}>{lang==='ru'?'Очистить заблокированных':'Clear blocked artists'}</button>}
              <div style={{padding:'14px 0 8px'}}><div style={{fontSize:10,color:TEXT_MUTED,marginBottom:7,textTransform:'uppercase' as const,letterSpacing:1}}>{t('language')}</div><div style={{display:'flex',gap:6}}>{(['ru','en'] as const).map(l=>(<button key={l} onClick={()=>chgLang(l)} style={{flex:1,padding:'9px',borderRadius:9,border:'none',background:lang===l?ACC:BG2,color:lang===l?BG:TEXT_SEC,fontSize:13,fontWeight:lang===l?600:400,cursor:'pointer',outline:'none'}}>{l==='ru'?'🇷🇺 Русский':'🇺🇸 English'}</button>))}</div></div>
              ...
              <button onClick={()=>{try{localStorage.clear();}catch{}setLiked([]);setPlaylists([]);setHistory([]);setRecs([]);setQueue([]);setFavArtists([]);setFavAlbums([]);setBlockedArtists([]);}} style={{width:'100%',marginTop:6,padding:'11px',background:'#1a0a0a',border:'1px solid #2a1010',borderRadius:11,color:'#c05050',fontSize:12,cursor:'pointer',outline:'none'}}>{t('resetData')}</button>
            </div>
          </div>
        )}
      </div>
      ...
      {current&&(
        <div style={{position:'fixed',bottom:NAV_H+5,left:8,right:8,background:MINI_BG,backdropFilter:'blur(20px)',border:'1px solid #3a3228',borderRadius:14,padding:'8px 12px 8px',zIndex:100}}>
          {/* Строка 1: обложка + инфо + громкость + плей */}
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:5}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:5,flexShrink:0}}>
              ...
              <div {...volSP} ref={volSP.ref} style={{width:38,height:16,display:'flex',alignItems:'center',cursor:'pointer',touchAction:'none'}}>
          ...
          {/* Строка 2: прогресс с перемоткой */}
          ...
          {/* #4 Строка 3: компактные кнопки Prev / Next */}
          <div style={{display:'flex',justifyContent:'center',alignItems:'center',gap:20}}>
            <button onClick={playPrev} style={{background:'none',border:'none',cursor:'pointer',padding:'2px 8px',display:'flex',alignItems:'center',gap:4,color:TEXT_MUTED,fontSize:10,outline:'none',opacity:playHistory.length>0?1:0.35}}>
            ...
            <button onClick={playNext} style={{background:'none',border:'none',cursor:'pointer',padding:'2px 8px',display:'flex',alignItems:'center',gap:4,color:TEXT_MUTED,fontSize:10,outline:'none',opacity:(queue.length>0||recs.length>0||history.length>0)?1:0.35}}>
      ...
      {screen!=='profile'&&screen!=='artist'&&screen!=='album'&&(
        <div style={{position:'fixed',bottom:0,left:0,right:0,background:'rgba(10,10,10,0.97)',backdropFilter:'blur(20px)',borderTop:'1px solid #1e1e1e',padding:'7px 0 13px',display:'flex',justifyContent:'space-around',zIndex:101,height:NAV_H}}>
          {NAV.map(item=>(<div key={item.id} onClick={()=>setScreen(item.id as 'home'|'search'|'library'|'trending')} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:3,cursor:'pointer',padding:'0 8px'}}>{item.icon(screen===item.id)}<span style={{fontSize:9,color:screen===item.id?ACC:'#606060',letterSpacing:0.3}}>{item.lbl()}</span></div>))}
        </div>
      )}
    </div>
  );
