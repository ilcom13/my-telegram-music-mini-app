        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',zIndex:540,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onPointerDown={()=>setDlWarnFor(null)}>
          <div onPointerDown={e=>e.stopPropagation()} style={{width:'100%',maxWidth:480,background:'#161616',borderRadius:'20px 20px 0 0',padding:'22px 20px calc(24px + env(safe-area-inset-bottom))',animation:'slideUp 0.25s ease both',border:'1px solid #252525',borderBottom:'none'}}>
            ...
            <div style={{fontSize:13,color:TEXT_SEC,lineHeight:1.55,marginBottom:18}}>
              {lang==='ru'?<>Этот режим работает <b style={{color:TEXT_PRIMARY}}>как для запланированных ситуаций</b> — поездок, перелётов, мест без интернета.<br/><br/>⚠️ <b style={{color:ACC}}>Только на устройствах Apple (iPhone, iPad):</b> если вы не заходите в мини-приложение более <b style={{color:TEXT_PRIMARY}}>7 дней подряд</b>, загруженные треки удалятся и их придётся скачать заново. При каждом заходе в мини-приложение этот таймер сбрасывается. На Android и ПК загрузки хранятся без ограничения по времени.</>
              :lang==='uk'?<>Цей режим працює <b style={{color:TEXT_PRIMARY}}>для запланованих ситуацій</b> — подорожей, перельотів, місць без інтернету.<br/><br/>⚠️ <b style={{color:ACC}}>Лише на пристроях Apple (iPhone, iPad):</b> якщо ви не заходите в міні-додаток понад <b style={{color:TEXT_PRIMARY}}>7 днів поспіль</b>, завантажені треки видаляться і їх доведеться завантажити знову. За кожного входу в міні-додаток цей таймер скидається. На Android і ПК завантаження зберігаються без обмеження в часі.</>
              :lang==='kk'?<>Бұл режим <b style={{color:TEXT_PRIMARY}}>жоспарланған жағдайларға</b> арналған — сапарлар, ұшулар, интернетсіз жерлер.<br/><br/>⚠️ <b style={{color:ACC}}>Тек Apple құрылғыларында (iPhone, iPad):</b> егер сіз мини-қолданбаға <b style={{color:TEXT_PRIMARY}}>7 күн қатарынан</b> кірмесеңіз, жүктелген тректер өшіріліп, оларды қайта жүктеу қажет болады. Мини-қолданбаға әр кіргенде бұл таймер қалпына келеді. Android және ДК-де жүктеулер уақыт шектеуінсіз сақталады.</>
              :lang==='pl'?<>Ten tryb działa <b style={{color:TEXT_PRIMARY}}>w zaplanowanych sytuacjach</b> — podróże, loty, miejsca bez internetu.<br/><br/>⚠️ <b style={{color:ACC}}>Tylko na urządzeniach Apple (iPhone, iPad):</b> jeśli nie otworzysz mini-aplikacji przez ponad <b style={{color:TEXT_PRIMARY}}>7 dni z rzędu</b>, pobrane utwory zostaną usunięte i trzeba będzie pobrać je ponownie. Przy każdym otwarciu mini-aplikacji licznik jest resetowany. Na Androidzie i komputerze pobrania są przechowywane bez ograniczeń czasowych.</>
              :lang==='tr'?<>Bu mod <b style={{color:TEXT_PRIMARY}}>planlı durumlar</b> içindir — seyahatler, uçuşlar, internetsiz yerler.<br/><br/>⚠️ <b style={{color:ACC}}>Yalnızca Apple cihazlarında (iPhone, iPad):</b> mini uygulamayı <b style={{color:TEXT_PRIMARY}}>üst üste 7 günden</b> fazla açmazsanız, indirilen parçalar silinir ve yeniden indirilmesi gerekir. Mini uygulamayı her açtığınızda bu sayaç sıfırlanır. Android ve PC'de indirmeler süre sınırı olmadan saklanır.</>
              :<>This mode is meant <b style={{color:TEXT_PRIMARY}}>for planned situations</b> — trips, flights, places without internet.<br/><br/>⚠️ <b style={{color:ACC}}>Apple devices only (iPhone, iPad):</b> if you don't open the mini app for more than <b style={{color:TEXT_PRIMARY}}>7 days in a row</b>, downloaded tracks will be deleted and you'll need to download them again. Each time you open the mini app, this timer resets. On Android and PC, downloads are kept without a time limit.</>}
            </div>
