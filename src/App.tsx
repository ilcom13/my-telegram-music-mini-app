...
const PlTrackRow=React.memo(function PlTrackRow({tr,i,displayName,displayArtistName,isActive,playing:isPlaying,isManualQ,curSort,editMode=false,onPlay,onQueue,onRemove,onMenu,onDragStart,onDrop,onMoveUp,onMoveDown}:PlTrackRowProps){
  ...
  return(
    <div
      ...
      onDrop={onDrop}
      onClick={handleClick}
      style={{position:'relative',touchAction:'pan-y',userSelect:'none' as const,borderBottom:'1px solid #111',overflow:'hidden',background:'transparent'}}
    >
      ...
      <div ref={innerRef} style={{display:'flex',alignItems:'center',gap:8,padding:'10px 12px 10px 14px',willChange:'transform',background:isActive?ACC_DIM:'transparent',borderRadius:12,margin:'0 4px'}}>
{editMode
...
export default function App(){
  ...
  // Автоочистка результатов треков при пустом запросе или при переходе в режим @
  useEffect(()=>{
    const q=query.trim();
...
const serverPlValid=Array.isArray(sv.playlists);
const plFromServer=serverPlTs>=localPlTs&&serverPlValid?sv.playlists:
  (localPlTs>serverPlTs?localPl:(serverPlValid?sv.playlists:localPl));
...
useEffect(()=>{
    ...
    fetch(`${W}/titles/load?uid=${uid}`).then(r=>r.json()).then(d=>{
      if(d.titles&&typeof d.titles==='object'){
        setCustomTitles(prev=>{const merged={...prev,...d.titles};try{localStorage.setItem('ctitles47',JSON.stringify(merged));}catch{}return merged;});
      }
    }).catch(()=>{});
  },[uid]);
  ...
  const saveRename=useCallback((trackId:string,newTitle:string,newArtist:string)=>{
    ...
    const tA=newArtist.replace(/[\u0000-\u001F\u007F]/g,'').trim().slice(0,200);
    setCustomTitles(prev=>{
      const next={...prev};
      const entry:{t?:string;a?:string}={};
      if(tT.length)entry.t=tT;
      if(tA.length)entry.a=tA;
      if(!entry.t&&!entry.a){delete next[trackId];}else{next[trackId]=entry;}
      try{localStorage.setItem('ctitles47',JSON.stringify(next));}catch{}
      fetch(`${W}/titles/save`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uid,titles:next})}).catch(()=>{});
      return next;
    });
  },[subActive,uid]);
...
const srcToProcess = originalSrcRef.current;
  ...
  try {
    ...
    const fxResp = await fetch(
      `https://eqq.onrender.com/fx-base64?preset=${preset}&speed=${speed}&reverb=${reverb}&pitch=${pitch}&bass=${bass}`,
      {
        method: 'POST',
...
const openAlbum=async(id:string,title:string,artist:string,cover:string)=>{
  ...
  const refreshSharedPl=async(pl:Playlist)=>{
    ...
    try{
      const r=await fetch(`${W}/pl/get?owner=${encodeURIComponent(pl.owner)}&id=${encodeURIComponent(pl.sourcePlId)}&t=${Date.now()}`,{cache:'no-store'});
      const d=await r.json();
    ...
    <div style={{position:'relative' as const,background:BG,height:'100vh',width:'100%',display:'flex',flexDirection:'column',alignItems:'center',padding:'0 22px',fontFamily:"-apple-system,'SF Pro Display',sans-serif",boxSizing:'border-box',overflow:'hidden',animation:'fadeIn 0.3s ease'}}>
      ...
      {current.cover&&<>
        <div style={{position:'absolute' as const,inset:0,zIndex:0,overflow:'hidden',pointerEvents:'none' as const}}>
          <img key={current.cover} src={current.cover} style={{position:'absolute' as const,inset:-40,width:'calc(100% + 80px)',height:'calc(100% + 80px)',objectFit:'cover',filter:'blur(60px) saturate(1.4) brightness(0.55)',transform:'scale(1.15)'}} alt="" onError={()=>{}}/>
        </div>
        <div style={{position:'absolute' as const,inset:0,zIndex:0,pointerEvents:'none' as const,background:`linear-gradient(to bottom,transparent 0%,transparent 30%,rgba(14,14,14,0.5) 65%,${BG} 100%)`}}/>
      </>}
      <audio ref={audio}/>
      <style>{`
...
<div key={tr.id+i} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 0',borderBottom:'1px solid #2a2a2a',transition:'background 0.15s ease'}}>
      ...
      <div style={{position:'relative' as const,zIndex:1,width:'100%',display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',paddingTop:20,paddingBottom:10,flexShrink:0}}>
        <button onClick={()=>{const p=progressRef.current;if(miniBarFillRef.current)miniBarFillRef.current.style.width=`${p}%`;if(miniBarThumbRef.current)miniBarThumbRef.current.style.left=`${p}%`;setFullPlayer(false);requestAnimationFrame(()=>requestAnimationFrame(()=>{window.scrollTo(0,savedScrollY.current);const plw=document.getElementById('pl-page-wrap');if(plw)plw.scrollTop=savedPlScrollY.current;}));}} style={{background:'none',border:'none',cursor:'pointer',padding:'8px 4px 8px 0',display:'flex',alignItems:'center',gap:6,justifySelf:'start',transition:'opacity 0.2s ease',...tap}}>
          <svg viewBox="0 0 24 24" style={{width:22,height:22,display:'block'}} fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
          <span style={{fontSize:15,fontWeight:500,color:'#fff'}}>{lang==='ru'?'Назад':lang==='uk'?'Назад':lang==='kk'?'Артқа':lang==='pl'?'Wstecz':lang==='tr'?'Geri':'Back'}</span>
        </button>
        <span style={{fontSize:11,color:'rgba(255,255,255,0.5)',letterSpacing:2.5,textTransform:'uppercase',fontWeight:600,whiteSpace:'nowrap'}}>{t('nowPlaying')}</span>
        <button onPointerDown={()=>setShowQueue(true)} style={{background:'rgba(255,255,255,0.06)',border:`1.5px solid ${queue.length>0?ACC+'88':'rgba(255,255,255,0.15)'}`,cursor:'pointer',width:40,height:40,borderRadius:'50%',position:'relative' as const,display:'flex',alignItems:'center',justifyContent:'center',justifySelf:'end',padding:0,transition:'all 0.2s ease',...tap}}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={queue.length>0?ACC:'#fff'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke 0.2s ease'}}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
      ...
      <div style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',justifyContent:'center',flexShrink:0,marginTop:'2vh',marginBottom:'3vh'}}>
        <div
          ...
          style={{borderRadius:22,overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,0.5)',position:'relative',cursor:'pointer',transition:'transform 0.3s cubic-bezier(0.25,0.46,0.45,0.94),box-shadow 0.3s ease'}}
        ...
        >
          <Img src={current.cover} size={Math.min(window.innerWidth-60, window.innerHeight*0.4, 360)} radius={0}/>
        </div>
      </div>
      <div style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,flexShrink:0,marginBottom:18,animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.05s both'}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:'clamp(20px,6vw,30px)',fontWeight:800,color:'#fff',lineHeight:1.15,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',marginBottom:3,letterSpacing:-0.5}}>{displayTitle(current)}</div>
          <button onClick={()=>{setFullPlayer(false);openArtist(current.permalink||'',current.artist,current.cover,0);}} style={{background:'none',border:'none',cursor:'pointer',padding:0,display:'block',textAlign:'left' as const,maxWidth:'100%',...tap}}>
            <span style={{fontSize:'clamp(13px,4vw,17px)',color:ACC,fontWeight:500,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',display:'block'}}>{displayArtist(current)}</span>
          </button>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
          <HBtn track={current} sz={28}/>
          <button onPointerDown={()=>setAddToPl(current)} style={{background:'rgba(255,255,255,0.08)',border:'none',cursor:'pointer',width:38,height:38,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',padding:0,transition:'background 0.2s ease',...tap}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/></svg>
          </button>
        </div>
      </div>
    ...
    {fxRemaining}/5 {lang==='ru'?'сегодня':lang==='uk'?'сьогодні':'today'} <span style={{fontSize:9,opacity:0.5}}>· {lang==='ru'?'1-я обработка дольше':lang==='uk'?'1-а обробка довша':lang==='kk'?'1-ші өңдеу ұзақ':lang==='pl'?'1. dłuższe':lang==='tr'?'1. daha uzun':'1st takes longer'}</span>
      ...
      <div style={{marginBottom:20}}>
        ...
        <input type="range" min="0.5" max="2" step="0.05" value={playbackSpeed}
          onChange={e=>{if(fxLoading)return;if(fxRemaining<=0)return;if(audio.current&&audio.current.duration>360)return;setPlaybackSpeed(parseFloat(e.target.value));}}
...
<input type="range" min="0" max="100" step="1" value={reverbAmount}
  onChange={e=>{if(fxLoading)return;if(!subActive||fxRemaining<=0)return;if(audio.current&&audio.current.duration>360)return;setReverbAmount(parseInt(e.target.value));}}
  style={{width:'100%',accentColor:ACC,cursor:'pointer',height:4}}/>
  <div style={{display:'flex',justifyContent:'space-between',marginTop:4}}>
...
<div style={{marginBottom:16}}>
  ...
  <input type="range" min="0.5" max="2" step="0.05" value={pitchAmount}
    onChange={e=>{if(fxLoading)return;if(!subActive||fxRemaining<=0)return;if(audio.current&&audio.current.duration>360)return;setPitchAmount(parseFloat(e.target.value));}}
    style={{width:'100%',accentColor:ACC,cursor:'pointer',height:4}}/>
  <div style={{display:'flex',justifyContent:'space-between',marginTop:4}}>
...
<div style={{marginBottom:16}}>
  ...
  <input type="range" min="0" max="12" step="1" value={bassAmount}
    onChange={e=>{if(fxLoading)return;if(!subActive||fxRemaining<=0)return;if(audio.current&&audio.current.duration>360)return;setBassAmount(parseInt(e.target.value));}}
    style={{width:'100%',accentColor:ACC,cursor:'pointer',height:4}}/>
  <div style={{display:'flex',justifyContent:'space-between',marginTop:4}}>
    <span style={{fontSize:10,color:TEXT_MUTED}}>0</span>
...
{showEqPanel&&(
      ...
      <div style={{width:'100%',flexShrink:0,marginBottom:2,animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.15s both'}}>
        ...
        <div style={{width:'100%',position:'relative',padding:'8px 0',margin:'-8px 0',cursor:'pointer',touchAction:'none'}}
          ...
          <div style={{width:'100%',height:4,background:'rgba(255,255,255,0.18)',borderRadius:2,position:'relative'}}>
            <div ref={seekBarFillRef} style={{height:'100%',background:ACC,borderRadius:2,width:'0%',pointerEvents:'none'}}/>
            <div ref={seekBarThumbRef} style={{position:'absolute',top:'50%',left:'0%',transform:'translate(-50%,-50%)',width:14,height:14,background:'#fff',borderRadius:'50%',pointerEvents:'none',boxShadow:'0 2px 6px rgba(0,0,0,0.4)'}}/>
          </div>
        </div>
        <div style={{position:'relative' as const,zIndex:1,display:'flex',justifyContent:'space-between',fontSize:13,color:'rgba(255,255,255,0.55)',marginTop:8,fontWeight:500}}>
      ...
      <div style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,marginTop:'2.5vh',marginBottom:'3vh',animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.18s both'}}>
        <button onPointerDown={()=>{if(!showFxPanel)fetch('https://eqq.onrender.com/health').catch(()=>{});setShowFxPanel(v=>!v);}} style={{background:'none',border:'none',cursor:'pointer',padding:8,transition:'opacity 0.2s ease',...tap}}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={showFxPanel?ACC:'rgba(255,255,255,0.85)'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke 0.2s ease'}}><line x1="4" y1="21" x2="4" y2="14"/><line x1="4" y1="10" x2="4" y2="3"/><line x1="12" y1="21" x2="12" y2="12"/><line x1="12" y1="8" x2="12" y2="3"/><line x1="20" y1="21" x2="20" y2="16"/><line x1="20" y1="12" x2="20" y2="3"/><line x1="1" y1="14" x2="7" y2="14"/><line x1="9" y1="8" x2="15" y2="8"/><line x1="17" y1="16" x2="23" y2="16"/></svg>
        </button>
        <button className="prev-next-btn" onPointerDown={e=>{e.preventDefault();if(fxLoading)return;playPrev();}} style={{background:'none',border:'none',cursor:'pointer',padding:8,opacity:fxLoading?0.2:playHistory.length>0?1:0.4,...tap}}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="#fff" stroke="#fff" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><path d="M19 20L9 12L19 4V20Z"/><rect x="4" y="5" width="2" height="14" rx="1"/></svg>
        </button>
        <button className="play-btn" onPointerDown={togglePlay} style={{width:'clamp(64px,18vw,80px)',height:'clamp(64px,18vw,80px)',borderRadius:'50%',background:ACC,border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0,boxShadow:`0 6px 24px ${ACC}66`,transition:'transform 0.15s ease',...tap}}>
          <PP sz="lg"/>
        </button>
        <button className="prev-next-btn" onPointerDown={e=>{e.preventDefault();if(fxLoading)return;playNext();}} style={{background:'none',border:'none',cursor:'pointer',padding:8,opacity:fxLoading?0.2:(queue.length>0||recs.length>0||history.length>0)?1:0.4,...tap}}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="#fff" stroke="#fff" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><path d="M5 4L15 12L5 20V4Z"/><rect x="18" y="5" width="2" height="14" rx="1"/></svg>
        </button>
        <button onPointerDown={()=>setLoop(!loop)} style={{background:'none',border:'none',cursor:'pointer',padding:8,transition:'opacity 0.2s ease',...tap}}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={loop?ACC:'rgba(255,255,255,0.85)'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke 0.2s ease'}}><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 014-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 01-4 4H3"/></svg>
        </button>
      </div>

      {/* Громкость */}
      <div style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',alignItems:'center',gap:10,flexShrink:0,marginBottom:'2.5vh',animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.22s both'}}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/></svg>
        <SliderTrack sp={volSP} h={3}/>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 010 7.07"/><path d="M19.07 4.93a10 10 0 010 14.14"/></svg>
      </div>

      {/* Нижний ряд: Playlists / Queue / Share */}
      <div style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',justifyContent:'space-around',alignItems:'center',flexShrink:0,marginBottom:'2vh',paddingBottom:'env(safe-area-inset-bottom)',animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.26s both'}}>
        <button onPointerDown={()=>setAddToPl(current)} style={{background:'none',border:'none',cursor:'pointer',display:'flex',flexDirection:'column' as const,alignItems:'center',gap:6,padding:'8px 12px',color:'rgba(255,255,255,0.55)',transition:'color 0.2s ease',...tap}}>
        ...
        <button onPointerDown={()=>setShowQueue(true)} style={{background:'none',border:'none',cursor:'pointer',display:'flex',flexDirection:'column' as const,alignItems:'center',gap:6,padding:'8px 12px',color:'rgba(255,255,255,0.55)',transition:'color 0.2s ease',...tap}}>
        ...
        <button onPointerDown={()=>shareTrack(current)} style={{background:'none',border:'none',cursor:'pointer',display:'flex',flexDirection:'column' as const,alignItems:'center',gap:6,padding:'8px 12px',color:'rgba(255,255,255,0.55)',transition:'color 0.2s ease',...tap}}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v12"/><polyline points="7 8 12 3 17 8"/><path d="M5 13v6a2 2 0 002 2h10a2 2 0 002-2v-6"/></svg>
          <span style={{fontSize:13,fontWeight:500}}>{lang==='ru'?'Поделиться':lang==='uk'?'Поділитися':lang==='kk'?'Бөлісу':lang==='pl'?'Udostępnij':lang==='tr'?'Paylaş':'Share'}</span>
        </button>
      </div>
          ...
          <div className="screen-fade" style={{position:'relative'}}>
            ...
            <div style={{position:'relative' as const,zIndex:1,padding:'14px 16px 6px',display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12}}>
              ...
              <button onClick={()=>setScreen('profile')} style={{position:'relative' as const,width:38,height:38,minWidth:38,borderRadius:'50%',padding:0,background:'none',border:'1.5px solid rgba(255,255,255,0.18)',cursor:'pointer',flexShrink:0,overflow:'hidden',...tap}}>
                ...
                {notifUnread>0&&<div style={{position:'absolute' as const,top:-2,right:-2,width:10,height:10,borderRadius:'50%',background:'#e74c3c',border:'2px solid #0e0e0e',boxSizing:'border-box' as const}}/>}
              </button>
            </div>
            ...
            {pinnedPlId&&playlists.find(p=>p.id===pinnedPlId)&&(()=>{
              ...
              <div style={{padding:'0 16px',marginBottom:14,animation:'slideUp 0.3s ease both'}}>
                ...
                <div onClick={()=>{setScreen('library');setLibTab('playlists');setOpenPlPage(pp.id);}} className="press-scale" style={{position:'relative' as const,overflow:'hidden',borderRadius:14,padding:'13px 14px',display:'flex',alignItems:'center',gap:12,cursor:'pointer',background:PINNED_BG_URL?'#1a1a1a':`linear-gradient(135deg,${ACC_DIM},rgba(239,191,127,0.06))`,border:'1px solid rgba(255,255,255,0.06)',minHeight:88,...tap}}>
                  ...
                  <div style={{position:'relative' as const,zIndex:1,width:62,height:62,borderRadius:8,overflow:'hidden',flexShrink:0,display:'grid',gridTemplateColumns:'1fr 1fr',gap:1,background:BG3,boxShadow:'0 3px 12px rgba(0,0,0,0.4)'}}>
                    {getPlCovers(pp).map((tr,i)=><div key={i} style={{overflow:'hidden'}}><Img src={tr.cover} size={31} radius={0}/></div>)}
                  ...
                  <div style={{position:'relative' as const,zIndex:1,flex:1,minWidth:0}}>
                    <div style={{fontSize:20,fontWeight:800,color:'#fff',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',letterSpacing:-0.4,marginBottom:2}}>{pp.name}</div>
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.7)'}}>{pp.tracks.length} {lang==='ru'?'треков':lang==='uk'?'треків':'tracks'}</div>
                  </div>
                  <div style={{position:'relative' as const,zIndex:1,flexShrink:0,width:36,height:36,borderRadius:'50%',background:ACC,display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 3px 10px rgba(239,191,127,0.3)'}}>
                    <svg viewBox="0 0 24 24" style={{width:16,height:16}} fill="none" stroke={BG} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                  </div>
                </div>
              </div>
              );
            ...
            <div style={{padding:'0 16px',marginBottom:14,animation:'slideUp 0.3s cubic-bezier(0.25,0.46,0.45,0.94) 0.1s both'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                <div style={{fontSize:15,fontWeight:700,color:'#fff',letterSpacing:-0.3}}>{lang==='ru'?'Для тебя':lang==='uk'?'Для тебе':lang==='kk'?'Сен үшін':lang==='pl'?'Dla Ciebie':lang==='tr'?'Senin için':'For you'}</div>
                <button onPointerDown={()=>setScreen('trending')} style={{display:'inline-flex',alignItems:'center',gap:2,background:'none',border:'none',color:ACC,fontSize:11,fontWeight:500,cursor:'pointer',padding:0,...tap}}>
                  {lang==='ru'?'Все':lang==='uk'?'Усі':lang==='kk'?'Барлығы':lang==='pl'?'Wszystkie':lang==='tr'?'Tümü':'See all'}
                  <svg viewBox="0 0 24 24" style={{width:11,height:11}} fill="none" stroke={ACC} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                </button>
              </div>
...
{forYouTracks.map((tr,i)=>(
  <TRow key={tr.id+'fy'+i} {...mkTRow(tr,{num:i+1,showBlockBtn:true,onArtistClick:(n,c,id)=>openArtist(id||'',n,c,0)})} onPlay={()=>{playTrack(tr);setQueue(forYouTracks.slice(i+1).filter(t=>t.mp3&&t.id!==tr.id));}} onMenu={(r:DOMRect)=>{setMenuAnchor({top:r.bottom+6,right:window.innerWidth-r.right+r.width/2,showBlock:true});setMenuId(tr.id);}} onCloseMenu={()=>{setMenuId(null);setMenuAnchor(null);}}/>
            ...
            {(()=>{
              ...
              return(
                <div style={{position:'relative'}}>
                  ...
                  {(()=>{const _today=new Date();const _showPrev=false;const mStat=_showPrev?monthStats.prev!:monthStats.current;const mTop=Object.entries(mStat.trackPlays).sort((a,b)=>b[1].count-a[1].count);const mSec=(s:number)=>{const h=Math.floor(s/3600);const m2=Math.floor((s%3600)/60);return h>0?`${h}h ${m2}m`:`${m2}m`;};return(
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',zIndex:540,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onPointerDown={()=>setDlWarnFor(null)}>
          <div onPointerDown={e=>e.stopPropagation()} style={{width:'100%',maxWidth:480,background:'#161616',borderRadius:'20px 20px 0 0',padding:'22px 20px calc(24px + env(safe-area-inset-bottom))',animation:'slideUp 0.25s ease both',border:'1px solid #252525',borderBottom:'none'}}>
            ...
            <div style={{fontSize:13,color:TEXT_SEC,lineHeight:1.55,marginBottom:18}}>
              {lang==='ru'?<><b style={{color:ACC}}>⚠️ Важно:</b> так как это Telegram-бот, вам нужно зайти в мини-приложение, пока у вас есть интернет (даже плохой — главное зайти в бота). После этого вы можете слушать скачанные треки сколько угодно без интернета.<br/><br/><span style={{color:TEXT_MUTED}}>На слабых телефонах или с малым объёмом ОЗУ бот может подтормаживать во время скачивания — это нормально и проходит, как только загрузка завершится.<br/><br/>На устройствах Apple (iPhone, iPad): если вы не заходите в мини-приложение более 7 дней подряд, загруженные треки удалятся и их придётся скачать заново. При каждом заходе таймер сбрасывается. На Android и ПК загрузки хранятся без ограничения по времени.</span></>
              :lang==='uk'?<><b style={{color:ACC}}>⚠️ Важливо:</b> оскільки це Telegram-бот, вам потрібно зайти в міні-додаток, поки у вас є інтернет (навіть поганий — головне зайти в бота). Після цього ви можете слухати завантажені треки скільки завгодно без інтернету.<br/><br/><span style={{color:TEXT_MUTED}}>На слабких телефонах або з малим обсягом ОЗП бот може підгальмовувати під час завантаження — це нормально і минає, щойно завантаження завершиться.<br/><br/>На пристроях Apple (iPhone, iPad): якщо ви не заходите в міні-додаток понад 7 днів поспіль, завантажені треки видаляються і їх доведеться завантажити знову. За кожного входу таймер скидається. На Android і ПК завантаження зберігаються без обмеження в часі.</span></>
              :lang==='kk'?<><b style={{color:ACC}}>⚠️ Маңызды:</b> бұл Telegram-бот болғандықтан, интернетіңіз бар кезде мини-қолданбаға кіруіңіз керек (нашар болса да — негізгісі ботқа кіру). Содан кейін жүктелген тректерді интернетсіз қалағаныңызша тыңдай аласыз.<br/><br/><span style={{color:TEXT_MUTED}}>Әлсіз телефондарда немесе жедел жады аз құрылғыларда жүктеу кезінде бот баяулауы мүмкін — бұл қалыпты жағдай және жүктеу аяқталған соң өтеді.<br/><br/>Apple құрылғыларында (iPhone, iPad): егер сіз мини-қолданбаға 7 күн қатарынан кірмесеңіз, жүктелген тректер өшіріліп, оларды қайта жүктеу қажет болады. Әр кіргенде таймер қалпына келеді. Android және ДК-де жүктеулер уақыт шектеуінсіз сақталады.</span></>
              :lang==='pl'?<><b style={{color:ACC}}>⚠️ Ważne:</b> ponieważ to bot Telegram, musisz otworzyć mini-aplikację, gdy masz internet (nawet słaby — ważne, żeby wejść do bota). Potem możesz słuchać pobranych utworów bez internetu, ile chcesz.<br/><br/><span style={{color:TEXT_MUTED}}>Na słabszych telefonach lub z małą ilością RAM bot może działać wolniej podczas pobierania — to normalne i ustępuje po zakończeniu pobierania.<br/><br/>Na urządzeniach Apple (iPhone, iPad): jeśli nie otworzysz mini-aplikacji przez ponad 7 dni z rzędu, pobrane utwory zostaną usunięte i trzeba będzie pobrać je ponownie. Przy każdym otwarciu licznik jest resetowany. Na Androidzie i komputerze pobrania są przechowywane bez ograniczeń czasowych.</span></>
              :lang==='tr'?<><b style={{color:ACC}}>⚠️ Önemli:</b> bu bir Telegram botu olduğu için, internetiniz varken (zayıf olsa bile — önemli olan bota girmek) mini uygulamayı açmanız gerekir. Bundan sonra indirilen parçaları internetsiz olarak istediğiniz kadar dinleyebilirsiniz.<br/><br/><span style={{color:TEXT_MUTED}}>Zayıf telefonlarda veya az RAM'li cihazlarda indirme sırasında bot yavaşlayabilir — bu normaldir ve indirme bittiğinde geçer.<br/><br/>Apple cihazlarında (iPhone, iPad): mini uygulamayı üst üste 7 günden fazla açmazsanız, indirilen parçalar silinir ve yeniden indirilmesi gerekir. Her açılışta sayaç sıfırlanır. Android ve PC'de indirmeler süre sınırı olmadan saklanır.</span></>
              :<><b style={{color:ACC}}>⚠️ Important:</b> since this is a Telegram bot, you need to open the mini app while you have internet (even a weak connection — the main thing is to open the bot). After that, you can listen to downloaded tracks offline as much as you want.<br/><br/><span style={{color:TEXT_MUTED}}>On low-end phones or devices with little RAM, the bot may lag during downloading — this is normal and goes away once the download finishes.<br/><br/>On Apple devices (iPhone, iPad): if you don't open the mini app for more than 7 days in a row, downloaded tracks will be deleted and you'll need to download them again. Each time you open it, the timer resets. On Android and PC, downloads are kept without a time limit.</span></>}
            </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:520,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onPointerDown={()=>setRenameTrack(null)}>
          <div onPointerDown={e=>e.stopPropagation()} style={{width:'100%',maxWidth:480,background:'#161616',borderRadius:'20px 20px 0 0',padding:'20px 18px calc(20px + env(safe-area-inset-bottom))',animation:'slideUp 0.25s ease both',border:'1px solid #252525',borderBottom:'none'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
              <Img src={renameTrack.cover} size={44} radius={8}/>
              <div style={{minWidth:0,flex:1}}>
                <div style={{fontSize:11,color:TEXT_MUTED,marginBottom:2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{renameTrack.artist}</div>
                <div style={{fontSize:13,fontWeight:600,color:TEXT_PRIMARY,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{renameTrack.title}</div>
              </div>
            </div>
            <div style={{fontSize:13,fontWeight:600,color:TEXT_PRIMARY,marginBottom:8}}>{lang==='ru'?'Название':lang==='uk'?'Назва':lang==='kk'?'Атауы':lang==='pl'?'Nazwa':lang==='tr'?'Ad':'Title'}</div>
            <input autoFocus value={renameVal} maxLength={200} onChange={e=>setRenameVal(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){saveRename(renameTrack.id,renameVal,renameArtistVal);setRenameTrack(null);}}} placeholder={renameTrack.title} style={{width:'100%',boxSizing:'border-box' as const,padding:'13px 14px',background:'#0e0e0e',border:`1px solid ${ACC}55`,borderRadius:12,color:TEXT_PRIMARY,fontSize:15,outline:'none',marginBottom:14}}/>
            <div style={{fontSize:13,fontWeight:600,color:TEXT_PRIMARY,marginBottom:8}}>{lang==='ru'?'Исполнитель':lang==='uk'?'Виконавець':lang==='kk'?'Орындаушы':lang==='pl'?'Wykonawca':lang==='tr'?'Sanatçı':'Artist'}</div>
            <input value={renameArtistVal} maxLength={200} onChange={e=>setRenameArtistVal(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){saveRename(renameTrack.id,renameVal,renameArtistVal);setRenameTrack(null);}}} placeholder={renameTrack.artist} style={{width:'100%',boxSizing:'border-box' as const,padding:'13px 14px',background:'#0e0e0e',border:`1px solid ${ACC}55`,borderRadius:12,color:TEXT_PRIMARY,fontSize:15,outline:'none',marginBottom:14}}/>
            <div style={{display:'flex',gap:10}}>
              {customTitles[renameTrack.id]&&(
                <button onPointerDown={()=>{saveRename(renameTrack.id,'','');setRenameTrack(null);}} style={{flex:'0 0 auto',padding:'13px 16px',background:'#1a1a1a',border:'1px solid #2a2a2a',borderRadius:12,color:'#d06060',fontSize:14,fontWeight:600,cursor:'pointer',...tap}}>{lang==='ru'?'Сбросить':lang==='uk'?'Скинути':lang==='kk'?'Қалпына келтіру':lang==='pl'?'Resetuj':lang==='tr'?'Sıfırla':'Reset'}</button>
              )}
              <button onPointerDown={()=>{saveRename(renameTrack.id,renameVal,renameArtistVal);setRenameTrack(null);}} style={{flex:1,padding:'13px',background:ACC,border:'none',borderRadius:12,color:BG,fontSize:14,fontWeight:700,cursor:'pointer',...tap}}>{lang==='ru'?'Сохранить':lang==='uk'?'Зберегти':lang==='kk'?'Сақтау':lang==='pl'?'Zapisz':lang==='tr'?'Kaydet':'Save'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showRenameLocked&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:520,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onPointerDown={()=>setShowRenameLocked(false)}>
          <div onPointerDown={e=>e.stopPropagation()} style={{width:'100%',maxWidth:480,background:'#141414',borderRadius:'24px 24px 0 0',padding:'24px 20px calc(28px + env(safe-area-inset-bottom))',animation:'slideUp 0.25s ease both',border:'1px solid #252525',borderBottom:'none',display:'flex',flexDirection:'column' as const,alignItems:'center',gap:10}}>
            <div style={{width:40,height:4,background:'#333',borderRadius:2,marginBottom:8}}/>
            <div style={{fontSize:28}}>⭐</div>
            <div style={{fontSize:16,fontWeight:800,color:TEXT_PRIMARY,textAlign:'center' as const}}>Premium only</div>
            <div style={{fontSize:13,color:TEXT_MUTED,textAlign:'center' as const,maxWidth:280,lineHeight:1.4}}>{lang==='ru'?'Переименование треков доступно только с подпиской Premium':lang==='uk'?'Перейменування треків доступне лише з Premium':lang==='kk'?'Тректердің атын өзгерту тек Premium-мен қолжетімді':lang==='pl'?'Zmiana nazw utworów dostępna tylko z Premium':lang==='tr'?'Yeniden adlandırma yalnızca Premium ile':'Renaming tracks is available only with Premium'}</div>
            <button onPointerDown={()=>{setShowRenameLocked(false);setShowPremium(true);}} style={{marginTop:6,padding:'13px 32px',background:ACC,border:'none',borderRadius:12,color:BG,fontSize:14,fontWeight:700,cursor:'pointer',...tap}}>{lang==='ru'?'Подключить Premium':lang==='uk'?'Підключити Premium':lang==='kk'?'Premium қосу':lang==='pl'?'Aktywuj Premium':lang==='tr'?'Premium Al':'Get Premium'}</button>
            <button onPointerDown={()=>setShowRenameLocked(false)} style={{padding:'8px',background:'none',border:'none',color:TEXT_MUTED,fontSize:13,cursor:'pointer',...tap}}>{lang==='ru'?'Закрыть':lang==='uk'?'Закрити':lang==='kk'?'Жабу':lang==='pl'?'Zamknij':lang==='tr'?'Kapat':'Close'}</button>
          </div>
        </div>
      )}
...
// До 25 числа показываем итоги прошлого месяца, после 25 — текущего
      ...
      {openPlPage&&(()=>{
        ...
        if(!pl)return null;
        if(pl.shared){refreshSharedPl(pl);}
...
<button onPointerDown={()=>{
          ...
          if(el){el.style.animation='slideOutRight 0.25s cubic-bezier(0.4,0,0.2,1) both';}
