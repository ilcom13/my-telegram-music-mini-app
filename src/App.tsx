...
if(pl&&pl.repeat&&pl.tracks.length>0){
    ...
    const onPauseSilence=()=>{const sil=silenceRef.current;if(sil&&sil.paused)sil.play().catch(()=>{});};
    a.addEventListener('timeupdate',onT);a.addEventListener('ended',onE);a.addEventListener('pause',onPauseSilence);
    return()=>{a.removeEventListener('timeupdate',onT);a.removeEventListener('ended',onE);a.removeEventListener('pause',onPauseSilence);};
  },[current,loop]);
