...
{showEqPanel&&(
      ...
      <div className="fp-controls" style={{position:'relative' as const,zIndex:1,width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,marginTop:'2vh',marginBottom:'2.5vh',animation:'slideUp 0.35s cubic-bezier(0.25,0.46,0.45,0.94) 0.18s both'}}>
        <button onPointerDown={()=>{if(!showFxPanel)fetch('https://eqq.onrender.com/health').catch(()=>{});setShowFxPanel(v=>!v);}} style={{background:'none',border:'none',cursor:'pointer',padding:4,opacity:showFxPanel?0.90:0.25,transition:'opacity 0.2s ease',...tap}}>
        ...
        <button onPointerDown={()=>setLoop(!loop)} style={{background:'none',border:'none',cursor:'pointer',padding:6,opacity:loop?0.90:0.25,transition:'opacity 0.2s ease',...tap}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={loop?ACC:'rgba(255,255,255,0.95)'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke 0.2s ease',width:16,height:16}}><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 014-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 01-4 4H3"/></svg>
        </button>
      </div>
