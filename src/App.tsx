...
export default function App() {
  ...
  // ── Sync helpers ──────────────────────────────────────────────────────────
  const syncSave = async (data: { liked: Track[]; playlists: Playlist[]; history: Track[]; volume: number }) => {
  ...
  const syncLoad = async () => {
    if (userId === 'anon') return;
    try {
      const r = await fetch(`${WORKER_URL}/sync/load?uid=${userId}`);
  ...
  useEffect(() => {
    ...
    // Try sync first, fallback to localStorage
    syncLoad().catch(() => {
      try {
  ...
  const triggerSync = (liked: Track[], playlists: Playlist[], history: Track[], volume: number) => {
    if (syncTimer.current) clearTimeout(syncTimer.current);
    syncTimer.current = setTimeout(() => syncSave({ liked, playlists, history, volume }), 2000);
  };

  useEffect(() => {
    if (history.length < 2) return;
    const artists = [...new Set(history.map(tr => tr.artist).filter(a => {
      const al = a.toLowerCase();
      return !REMIX_WORDS_CLIENT.some(w => al.includes(w));
    }))].slice(0,5);
  ...
  const playTrackDirect = (track: Track) => {
    ...
    setHistory(prev => {
      ...
      triggerSync(liked, playlists, n, volume);
      return n;
    });
  };

  const playTrack = (track: Track) => {
    if (track.isArtist) { setQuery(track.title); setSearchMode('sound'); setScreen('search'); setTimeout(doSearch, 100); return; }
    if (!track.mp3) return;
  ...
  const toggleLike = (track: Track, e?: React.MouseEvent) => {
    ...
    setLiked(prev => {
      ...
      triggerSync(n, playlists, history, volume);
      return n;
    });
  };
  ...
  const setVol = (v: number) => {
    setVolume(v);
    try { localStorage.setItem('vol47', String(v)); } catch {}
  };
  ...
  const createPlaylist = () => {
    ...
    setPlaylists(prev => {
      ...
      triggerSync(liked, n, history, volume);
      return n;
    });
  ...
  const addToPlaylist = (plId: string, track: Track) => {
    setPlaylists(prev => {
      const n=prev.map(pl=>pl.id===plId&&!pl.tracks.some(tr=>tr.id===track.id)?{...pl,tracks:[...pl.tracks,track]}:pl);
      try{localStorage.setItem('playlists47',JSON.stringify(n));}catch{}
      triggerSync(liked, n, history, volume);
      return n;
    });
  ...
  const shufflePlaylist = (pl: Playlist) => {
    ...
    if (!sh.length) return;
    playTrack(sh[0]); setQueue(sh.slice(1));
  };
  ...
  const moveQueue = (from: number, to: number) => {
    setQueue(prev => {
      ...
      try{localStorage.setItem('queue47',JSON.stringify(n));}catch{}
      return n;
    });
  };
    ...
    return (
      <div onClick={()=>playTrack(track)} style={{display:'flex',alignItems:'center',gap:12,padding:'9px 12px',borderRadius:14,cursor:'pointer',marginBottom:2,background:active?ACC_DIM:'transparent'}}>
        ...
        <div style={{position:'relative',flexShrink:0}}>
          <Cover cover={track.cover} size={48} radius={track.isArtist?24:11}/>
          {active && !track.isArtist && <div style={{position:'absolute',inset:0,borderRadius:11,background:'rgba(0,0,0,0.45)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>{playing?'⏸':'▶'}</div>}
        </div>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:5,marginTop:2}}>
            <span style={{fontSize:12,color:track.isArtist?ACC:'#666',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:120}}>{track.artist}</span>
            {!track.isArtist && track.plays>0 && <span style={{fontSize:11,color:'#444',flexShrink:0}}>· {fmtPlays(track.plays)}</span>}
          </div>
        </div>
        {!track.isArtist && <>
          <HeartBtn track={track}/>
          <button onClick={e=>addToQueue(track,e)} title={t('addToQueue')} style={{background:'none',border:'none',cursor:'pointer',padding:4,flexShrink:0}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#555" strokeWidth="2" strokeLinecap="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><circle cx="3" cy="6" r="1" fill="#555"/><circle cx="3" cy="12" r="1" fill="#555"/><circle cx="3" cy="18" r="1" fill="#555"/></svg>
          </button>
        ...
        {track.isArtist && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#555" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>}
      </div>
    );
  };
  ...
  if (fullPlayer && current) return (
    <div style={{background:'#0c0c11',minHeight:'100vh',width:'100%',display:'flex',flexDirection:'column',alignItems:'center',padding:'0 24px 40px',fontFamily:"-apple-system,'SF Pro Display',sans-serif",boxSizing:'border-box'}}>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:200,display:'flex',alignItems:'flex-end'}} onClick={()=>setShowQueue(false)}>
          <div style={{background:'#161622',width:'100%',borderRadius:'20px 20px 0 0',padding:'20px 16px 40px',maxHeight:'72vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
              <div>
                <div style={{fontSize:16,fontWeight:600,color:'#f0f0f8'}}>{t('queue')}</div>
                <div style={{fontSize:12,color:'#555',marginTop:2}}>{queue.length} треков · перетащи для изменения порядка</div>
              </div>
              <button onClick={()=>setQueue([])} style={{background:'none',border:'none',cursor:'pointer',fontSize:12,color:'#555'}}>Очистить</button>
            </div>
            {queue.length===0
              ? <div style={{color:'#555',fontSize:14,textAlign:'center',padding:'24px 0'}}>Очередь пустая</div>
              : queue.map((tr,i)=>(
                <div key={tr.id+i}
                  draggable
                  onDragStart={()=>setDragIdx(i)}
                  onDragOver={e=>{e.preventDefault();}}
                  onDrop={()=>{if(dragIdx!==null&&dragIdx!==i){moveQueue(dragIdx,i);}setDragIdx(null);}}
                  style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:'1px solid #1e1e2e',cursor:'grab',background:dragIdx===i?'rgba(226,142,254,0.08)':'transparent',borderRadius:8,transition:'background 0.1s'}}>
                  <div style={{color:'#333',fontSize:18,padding:'0 4px',cursor:'grab'}}>⠿</div>
      ...
      {/* Actions: like | playlist | queue ……… loop | share */}
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:36,marginTop:20,width:'100%'}}>
        <button style={{background:'none',border:'none',cursor:'pointer',opacity:0.35}}>
        ...
        <button onClick={()=>{if(queue.length>0){const next=queue[0];setQueue(prev=>prev.slice(1));playTrackDirect(next);}}} style={{background:'none',border:'none',cursor:'pointer',opacity:queue.length>0?1:0.35}}>
      ...
      <div style={{paddingBottom: current ? NAV_H+72+8 : NAV_H+8, minHeight:'100vh'}}>
          ...
          <div>
            <div style={{padding:'48px 16px 12px'}}>
              ...
              <div style={{display:'flex',gap:10}}>
                <input type="text" placeholder={T[lang]['searchPlaceholder']||'Артист или трек...'} value={query}
              ...
              <div style={{fontSize:11,color:'#444',marginTop:6,paddingLeft:2}}>
                {searchMode==='sound' && 'Оригинальные треки'}
                {searchMode==='remix' && 'Speed up · Slowed · Reverb · Remix'}
                {searchMode==='artists' && 'Артисты и исполнители'}
              </div>
        ...
        {screen==='library' && (
          <div>
            ...
            {libTab==='liked' && (
              liked.length===0
                ? <div style={{display:'flex',flexDirection:'column',alignItems:'center',paddingTop:60,color:'#444'}}><div style={{fontSize:44,marginBottom:14}}>🎵</div><div style={{fontSize:15,color:'#555'}}>{t('noLiked')}</div></div>
                      ...
                      {isOpen && (
                        <div>
                          <div style={{display:'flex',gap:8,padding:'0 16px 12px'}}>
                            ...
                            <button onClick={()=>setPlaylists(prev=>{const n=prev.map(p=>p.id===pl.id?{...p,repeat:!p.repeat}:p);try{localStorage.setItem('playlists47',JSON.stringify(n));}catch{}triggerSync(liked,n,history,volume);return n;})}
          ...
          return (
            <div>
              ...
              {trendLoading&&currentTracks.length===0
                ? <div style={{display:'flex',alignItems:'center',justifyContent:'center',paddingTop:80}}><div style={{fontSize:14,color:'#555'}}>{t('loading')}</div></div>
        ...
        {screen==='profile' && (
          <div>
            ...
            <div style={{padding:'0 16px'}}>
              {[{label:t('likedTracks'),value:liked.length},{label:t('playlists'),value:playlists.length},{label:t('listenedTracks'),value:history.length}].map(item=>(
              ...
              <button onClick={()=>syncSave({liked,playlists,history,volume})} style={{width:'100%',marginTop:12,padding:'13px',background:ACC_DIM,border:`1px solid ${ACC}33`,borderRadius:14,color:ACC,fontSize:14,cursor:'pointer'}}>
